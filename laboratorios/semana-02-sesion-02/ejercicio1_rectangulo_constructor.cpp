...
public:
    Rectangulo(double base_inicial, double altura_inicial) {
        if(!setBase(base_inicial)){
             base = 1.0;
                std::cout << "Aviso: base invalida, se usa 1.0 por defecto" << std::endl;
        }
        if(!setAltura(altura_inicial)){
             altura = 1.0;
